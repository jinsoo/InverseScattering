...
str1 = Sphere(shape = (250,250,250), center = (125,125,125), R = lamb*3/dx, eps=5, mu=1)
eps = str1.epsr[:,:,125]
plt.imshow(eps)
plt.show()
del str1
...
while iter < 120:
    ...
    gamma = ((np.linalg.norm(g) / np.linalg.norm(np.matmul(A,g)))).item()**2
    if iter % 20 == 0:
